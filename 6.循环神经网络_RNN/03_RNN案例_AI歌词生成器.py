...
def build_vocab():
    # 1. 每行文本分词结果 -> 去重后所有的词.
    unique_words, all_words = [], [] # unique_words = set()   set()是一个无序不重复元素集合
    # 2. 遍历数据集, 获取到每行文本.
...
class LyricsDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, corpus_idx, num_chars):
        ...
        # 1.2 每个句子中词的个数. （每个训练样本中词的个数）
        self.num_chars = num_chars
        # 1.3 文档数据中词的数量, 不去重.
        self.word_count = len(self.corpus_idx)
        # 1.4 句子数量 （数据库能构建多少个样本）
        # self.number = self.word_count // self.num_chars # 给的代码：0~31 32~63 64~95 ... 
        self.number = self.word_count - self.num_chars # 修改之后：0~31 1~32 2~33 ... 

    # 2. 当使用 len(obj)时, 自动调用此方法. 返回数据集的大小, 即样本数量
    def __len__(self):
        # 返回句子数量
        return self.number

    # 3. 当使用 obj[index]时, 自动调用此方法. 输入索引, 返回数据集中的一个样本（输入值和目标值 的张量形式）
...
class TextGenerator(nn.Module):
    # 1. 初始化方法 词嵌入层 循环网络层 输出层（全连接层）
    def __init__(self, unique_word_count):  # unique_word_count: 去重的词的数量(5703)
        # 1.1 初始化父类的成员.
        super().__init__()
        # 1.2 初始化词嵌入层: 语料中词的数量, 词向量的维度.
        self.ebd = nn.Embedding(unique_word_count, 128) # 5703 * 128
        # 1.3 循环网络层: 词向量维度：128, 隐藏层维度: 256, 网络层数: 1
        self.rnn = nn.RNN(128, 256, 1)
        # 1.4 输出层(全连接层):  特征向量维度(和隐藏向量维度一致), 词表中词的个数.
        # 隐藏状态 -> 5703个词的分数
        # 词1得分 词2得分 ... 词5703得分   argmax()
        self.out = nn.Linear(256, unique_word_count)    # 词表中每个词(5703个词)的概率 -> 选概率最大的哪个词作为 预测结果.

    # 2. 前向传播方法
    def forward(self, inputs, hidden):
        # 2.1 初始化 词嵌入层处理.
        # inputs: (batch句子的数量, 句子的长度)
        # embd格式: (batch句子的数量, 句子的长度, 词向量维度)
        embd = self.ebd(inputs) # (5,32) -> (5,32,128)

        # 2.2 rnn处理
        # rnn格式: (句子的长度, batch句子的数量, 隐藏层维度)
        # rnn：(32, 5, 128)   output: (32, 5, 256)   hidden: (1, 5, 256)
        # output: 每个时间步的输出, 包含了所有时间步的隐藏状态. 
        # hidden: 最后1个时间步的隐藏状态.                 
        ...
        # (32,5,256) -> (32*5,256) -> (32*5,5703)
        # 160个位置 预测5703个词的概率 -> 选概率最大的哪个词作为 预测结果.
        output = self.out(output.reshape(shape=(-1, output.shape[-1])))
        # 2.4 返回结果, 预测结果, 隐藏层.
        return output, hidden

    # 3. 隐藏层的初始化. 第一次输入时, 没有上一次的隐藏状态, 需要初始化一个全0的隐藏状态.
...
def train():
    ...
    lyrics_dataloader = DataLoader(lyrics, batch_size=5, shuffle=True, drop_last=True) # drop_last: 最后一个批次如果不足5个样本，就丢弃掉，不参与训练。
    # 5. 定义损失函数
    ...
    # 所有批次的训练结果，包括 批次数 总损失
    # start, iter_num, total_loss = time.time(), 0, 0.0
    # 7.2 具体的每轮训练动作.
    for epoch in range(epochs):     # epoch: 0, 1, 2, 3...49, 分别表示: 第1轮, 第2轮, ... 第50轮.
        ...
        for x, y in lyrics_dataloader:
            # 7.5 获取隐藏层初始值.
            # 可以直接在这里用全0的隐藏层，就不用写init_hidden()方法.
            # hidden = torch.zeros(1, x.shape[0], 256)  # x.shape[0] = batch_size = 5
            hidden = model.init_hidden(x.shape[0]) # model.init_hidden(5) 写死5，如果最后一个批次不足5个样本，就会越界
            # 7.6 模型计算.
            output, hidden = model(x, hidden)
            # 7.7 计算损失.
            # CrossEntropyLoss(input, target) N是样本数, C是类别数
            # input.shape = (N,C) target.shape = (N)
            # y: (5, 32) -> (32, 5) -> (160)
...
def evaluate(start_word, sentence_length):
    ...
    # 6. 定义列表, 存放: 产生的词的索引.
    generate_sentence = [word_idx]  # 开始词的索引是列表的第1个值.
    # 7. 遍历句子长度, 获取到每一个词.
