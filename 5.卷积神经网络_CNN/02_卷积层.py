...
def dm01():
    ...
    # 4. 因为这里只有1张图, 所以我们给它再增加1个维度, 从 CHW -> (1, C, H, W), 1张3通道的 640*640像素的 图
    ...
    # 6. 具体的卷积计算.
    conv_img = conv(img3) # conv: 卷积层对象, conv(img3): 卷积计算, 输入: img3, 输出: conv_img

    # 7. 打印卷积后的结果.   1张4通道的 319*319像素的 图
    # print(f'conv_img: {conv_img}, shape: {conv_img.shape}') # (1, 4, 319, 319)

    # 8. 查看提取到的 4个 特征图.
    img4 = conv_img[0] # (4, 319, 319) -> CHW
    ...
    # 10. 可视化第1个通道的特征图.
    feature1 = img5[:, :, 0].detach().numpy()           # 第0通道(即: 第1通道的)  (319, 319)像素图
    plt.imshow(feature1)
    plt.show()
